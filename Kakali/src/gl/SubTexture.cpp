...
SubTexture::SubTexture(const std::shared_ptr<Texture>& texture, glm::vec2 size, uint32_t slotX, uint32_t slotY)
...
void SubTexture::SetSubTexture(glm::vec2 size, uint32_t slotX = 0, uint32_t slotY = 0)
{
	m_size = size;
	SetSlot(slotX, slotY);
}

void SubTexture::SetSlot(uint32_t slotX, uint32_t slotY)
{
